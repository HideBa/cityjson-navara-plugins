/**
 * The COLOURS a city layer is drawn with, as a host-supplied parameter.
 *
 * Not to be confused with a CityJSON *appearance* (textures and materials
 * carried by the data, drawn through `AppearanceTheme`): these are the
 * plugin's own colours for what the data does not colour itself.
 *
 ...
 * `CityJSONPluginOptions.colors` / `FlatCityBufPluginOptions.colors`
 * (a per-plugin default) and `AddCityModelOptions.colors` /
 * `OpenStreamOptions.colors` (a per-layer override), so a host's brand
...
/** The values the plugins shipped with before the colours became a parameter. */
export const DEFAULT_CITY_COLORS: Required<
  Pick<CityColors, "highlightColor" | "hoverColor">
> &
  CityColors = {
...
export interface ResolvedCityColors extends HighlightRGB {
  readonly highlightColor: string;
  readonly hoverColor: string;
 ...
 * colours first, then the layer's. `undefined` entries are skipped, so a
 * caller can pass options straight through without guarding.
 */
export function resolveCityColors(
  ...overrides: ReadonlyArray<CityColors | undefined>
): ResolvedCityColors {
  let highlightColor = DEFAULT_CITY_COLORS.highlightColor;
  let hoverColor = DEFAULT_CITY_COLORS.hoverColor;
